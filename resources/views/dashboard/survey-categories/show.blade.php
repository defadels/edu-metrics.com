...
<div class="max-w-4xl">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-4">
        <div class="flex justify-between items-start mb-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Dibuat: {{ $category->created_at->format('d M Y') }}</p>
            </div>
            <div class="flex space-x-2">
                <a href="{{ route('dashboard.categories.edit', $category) }}" class="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 dark:bg-yellow-500">
                    Edit
                </a>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Survey dalam Kategori ini ({{ $category->surveys->count() }})</h4>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($category->surveys as $survey)
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $survey->title }}</td>
                                <td class="px-6 py-4">
        ...
        @else
            <p class="text-gray-500 dark:text-gray-400">Belum ada survey dalam kategori ini.</p>
        @endif
    </div>
    
    <div class="mt-4">
        <a href="{{ route('dashboard.categories.index') }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400">← Kembali ke daftar</a>
    </div>
</div>
@endsection
