...
@section('page-title', 'Edit Survey Category')

@section('content')
<div class="max-w-2xl animate-fade-in-up">
    <form action="{{ route('dashboard.categories.update', $category) }}" method="POST" class="card-modern p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea 
                ...
                placeholder="Survey category description...">{{ old('description', $category->description) }}</textarea>
            @error('description')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        </div>
