...
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow p-6 mb-4">
        <div class="flex justify-between items-start mb-4">
            <div>
                <h3 class="text-2xl font-bold text-gray-900">Question</h3>
            ...
            <div class="flex space-x-2">
                ...
                <form action="{{ route('dashboard.questions.destroy', $question) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this question?');">
                    @csrf
                    @method('DELETE')
        ...
        @if($question->likertScale)
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Likert Scale</label>
    ...
    @if($question->options->count() > 0)
        <div class="bg-white rounded-lg shadow p-6 mb-4">
            <h4 class="text-lg font-semibold text-gray-900 mb-4">Options ({{ $question->options->count() }})</h4>
            <div class="space-y-2">
                @foreach($question->options->sortBy('order') as $option)
                    <div class="border border-gray-200 rounded-lg p-3">
                        <p class="text-gray-900">{{ $option->option_text }}</p>
                        <p class="text-xs text-gray-500 mt-1">Order: {{ $option->order }}</p>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="mt-4">
        <a href="{{ route('dashboard.questions.index') }}" class="text-blue-600 hover:text-blue-900">← Back to list</a>
    </div>
</div>
@endsection
