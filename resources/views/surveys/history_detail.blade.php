                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider w-16 text-center">
                                No</th>
                                    ...
                                    @elseif($question->question_type === 'multiple_choice')
                                        <div class="space-y-2">
                                            @foreach ($question->options->sortBy('order') as $mcOption)
                                                @php
                                                    $isSelected =
                                                        $answer && $answer->selected_option_id == $mcOption->id;
                                                @endphp
                                                <div
                                                    class="flex items-center gap-3 p-3 border {{ $isSelected ? 'border-theme-primary bg-theme-primary/5' : 'border-gray-100 bg-gray-50 opacity-60' }} rounded-xl">
                                                    <div
                                                        class="w-5 h-5 flex items-center justify-center rounded-full border {{ $isSelected ? 'border-theme-primary' : 'border-gray-300 bg-white' }}">
                                                        @if ($isSelected)
                                                            <div class="w-2.5 h-2.5 bg-theme-primary rounded-full"></div>
                                                        @endif
                                                    </div>
                                                    <span
                                                        class="{{ $isSelected ? 'text-theme-primary font-bold' : 'text-gray-500' }}">
                                    ...
                                    @elseif($question->question_type === 'likert')
                                        <div class="grid grid-cols-1 sm:grid-cols-5 gap-2 mt-4">
                                            @foreach ($question->likertScale->options->sortBy('order') as $option)
                                                ...
                                                @endphp
                                                <div
                                                    class="flex flex-col items-center justify-center p-3 rounded-xl border {{ $isSelected ? 'border-theme-active bg-theme-active/5 shadow-sm' : 'border-gray-100 bg-gray-50 opacity-60' }}">
                                                    <div
                                                        class="w-6 h-6 flex items-center justify-center rounded-full border mb-2 {{ $isSelected ? 'border-theme-active bg-white' : 'border-gray-300 bg-white' }}">
                                                        @if ($isSelected)
                                                            <div class="w-3 h-3 bg-theme-active rounded-full"></div>
                                                        @endif
                                                    </div>
                                                    <span
                                                        class="text-xs text-center {{ $isSelected ? 'text-theme-active font-bold' : 'text-gray-500' }}">
                                                        {{ $option->label }}
                                                    </span>
                                                </div>
                                            @endforeach
